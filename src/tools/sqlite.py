...
from src.utils.helper import is_connected
from configs.paths import ACCOUNTANT_DB_FILE

# Update this path if needed

def get_connection():
  return sqlite3.connect(ACCOUNTANT_DB_FILE)
...
# ------------------------------------------------------------------
db = SQLDatabase.from_uri(f"sqlite:///{ACCOUNTANT_DB_FILE}")

agent_config = settings.get_agent_model_config("Accountant")
llm = ChatOllama(
  model=agent_config["model"] if is_connected() else agent_config["local_model"],
